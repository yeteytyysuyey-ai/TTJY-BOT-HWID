...
async function handleButton(interaction: ButtonInteraction) {
    ...
    switch (customId) {
        case 'keys':
            if (supabase) {
                const { data } = await supabase.from('keys').select('*').eq('discord_id', interaction.user.id);
                const payload = panelCommand.renderPanel({ page: 'keys', myKeys: data || [] }) as any;
                // Combine CV2 flag with Ephemeral flag (bitwise OR)
