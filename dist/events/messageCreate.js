...
async function handleMessageCreate(message) {
    ...
    if (command === 'help' || command === 'admin' || command === 'panel' || command === 'menu') {
        const embed = new discord_js_1.EmbedBuilder()
            ...
            value: [
                ...
                '• `!addhwid <Key> <HWID>` - ผูก HWID ให้ Key (Max 3)',
    ...
    if (command === 'gen' || command === 'create' || command === 'addkey') {
        ...
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .setColor('#00ff88')
                .addFields({ name: '👤 ลูกค้า', value: `<@${targetUserId}> (\`${targetUserId}\`)`, inline: true }, { name: '⏳ อายุ', value: `${days} วัน`, inline: true }, { name: '🏷️ ชื่อ Key', value: keyName, inline: false }, { name: '🔑 Key Value', value: `\`\`\`${generatedKey}\`\`\``, inline: false }, { name: '🖥️ HWID', value: 'ยังไม่ผูก (รอ User bind เอง)', inline: false }, { name: '📬 สถานะ DM', value: dmStatus, inline: false })
                .setTimestamp();
    ...
    if (command === 'addhwid' || command === 'bind') {
        ...
        try {
            ...
            if (!currentHwids.find((h) => (h.hwid_value || h) === hwidValue)) {
            ...
            await statusMsg.edit(`✅ **ผูก HWID สำเร็จ!**\nKey: \`${keyValue}\`\nHWID: \`${hwidValue}\`\n\n*(ระบบ Max 3 HWID per key — เขียนลง Database โดยตรง)*`);
        }
        catch (err) {
    ...
    if (command === 'delhwid' || command === 'removehwid') {
        ...
        try {
            ...
            if (hwidValue) {
                let currentHwids = keyRecord.hwids || [];
                newHwids = currentHwids.filter((h) => (h.hwid_value || h) !== hwidValue);
        ...
        try {
            ...
            for (const [idx, k] of userKeys.entries()) {
                ...
                if (k.hwids && k.hwids.length > 0) {
                    hwidDisplay = k.hwids.map((h) => `\`${h.hwid_value || h}\``).join(', ');
                }
                embed.addFields({
    ...
    if (command === 'key') {
        ...
        try {
            ...
            if (hwidsList.length > 0) {
                hwidDisplay = hwidsList.map((h, i) => `${i + 1}. ${h.custom_name ? `**${h.custom_name}**: ` : ''}\`${h.hwid_value || h}\``).join('\n');
            }
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle(`🔑 ข้อมูล Key: \`${keyValue}\``)
                .setColor('#ffaa00')
                .addFields({ name: '👤 เจ้าของ Key', value: keyRecord.discord_id ? `<@${keyRecord.discord_id}> (\`${keyRecord.discord_id}\`)` : 'ไม่ระบุ', inline: true }, { name: '🏷️ ชื่อ Key', value: keyRecord.custom_name || 'VIP Key', inline: true }, { name: `🖥️ HWID (${hwidsList.length}/3)`, value: hwidDisplay, inline: false }, { name: '📅 สร้างเมื่อ', value: keyRecord.created_at ? `<t:${Math.floor(new Date(keyRecord.created_at).getTime() / 1000)}:f>` : 'ไม่ทราบ', inline: false })
                .setTimestamp();
            await statusMsg.edit({ content: '', embeds: [embed] });
        }
        catch (err) {
    ...
    if (command === 'stats' || command === 'info') {
        ...
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .setColor('#00ff00')
                .addFields({ name: '👥 จำนวนลูกค้าทั้งหมด', value: `${uniqueUsers} คน`, inline: true }, { name: '🔑 จำนวน Key ทั้งหมด', value: `${totalKeys} คีย์`, inline: true }, { name: '🖥️ HWID ที่ผูกแล้ว', value: `${boundHwids}/${totalKeys}`, inline: true }, { name: '📋 รายชื่อลูกค้าและจำนวนคีย์', value: userListText || 'ยังไม่มีข้อมูล' })
                .setTimestamp();
